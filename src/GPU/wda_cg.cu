...
double *epsilon_device;
double *sigma_device;

double *tot_rho_device;
double *sigma_x3_device;
double *epsilon_x_device;
double *HS_diameter_3_device;
double *MBWR_x_device;
double *MBWR_gamma_device;
double *MBWR_a_device;
double *MBWR_b_device;
double *MBWR_c_device;
double *MBWR_d_device;
double *MBWR_F_device;
double *MBWR_G_device;
...
double *w_cor_FFT_device;


double *ncor_separate_device;
double *ncor_separate_imagine_device;
double *ncor_separate_device_temp_save;
...
cufftDoubleComplex *ncor_FFT_in_device;
...
cufftDoubleComplex *ncor_FFT_out_device;
...
__global__
void sum(int n, int fake_N, double *d_x, double *reduction, double *tmp_reduction_sum)
{
	// the number of variable claimed on shared memory equals to the number of blocks assigned
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int loc_t = threadIdx.x;
	int step;
	for (int i=index; i<fake_N; i+=stride)
	{
		if(i<n)
		{
			tmp_reduction_sum[index] = 1.0 * d_x[i];
		}
		else
		{
			tmp_reduction_sum[index] = 0;
		}
		__syncthreads();
		for (step=blockDim.x/2; step>0; step/=2)
		{
			if (loc_t<step)
			{
				tmp_reduction_sum[index] += tmp_reduction_sum[index+step];
				__syncthreads();
			}
		}
		if (threadIdx.x == 0)
		{
			reduction[blockIdx.x] = tmp_reduction_sum[index];
		}
	}
}
	...
	double *w_cor_FFT_device)
{
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]*k_gas_device[0]); i+=stride)
	{
		...
		if ((i%(na_device[0]*nb_device[0]*nc_device[0])) == 0)
		{
			...
			w_cor_FFT_device[i] = 1.0 / (na_device[0]*nb_device[0]*nc_device[0]);
		}
		else
		{
			...
			fft_abs_kD = 1.0*fabs(fft_k)*diameter_HS_device[index_gas];
			...
			w_cor_FFT_device[i] = 3.0*(sin(fft_abs_kD)-fft_abs_kD*cos(fft_abs_kD))/pow(fft_abs_kD,3) / (na_device[0]*nb_device[0]*nc_device[0]);
	...
	double *diameter_HS_device, double *sqrt_dens_device, double *cutoff_device, double *Vext_device)
{
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]*k_gas_device[0]); i+=stride)
	{
		...
		// dens_device[i] = rho_bulk_device[index_gas];
...
__global__
void real_split_into_complex_cor_batch(int n, double *in, cufftDoubleComplex *out, int num_batch)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i;
...
__global__
void complext_split_into_real_only_separate_batch(int n, double *out_r, cufftDoubleComplex *in, int num_batch)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i;
...
void add_weighted_density(int *na_device, int *nb_device, int *nc_device, double *diameter_HS_device, int *k_gas_device, 
	...
	cufftDoubleComplex *ncor_FFT_in_device, 
	double *w_n0_FFT_device, double *w_n3_FFT_device, double *w_nv2x_FFT_device, double *w_nv2y_FFT_device, double *w_nv2z_FFT_device, 
	double *w_cor_FFT_device, double *uatt_device, double *denskr_device, double *denski_device, int j)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i, jj;
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]); i+=stride)
	{
		...
		ncor_FFT_in_device[i].x = FFT_out_device[i].x*w_cor_FFT_device[batch_i];
		ncor_FFT_in_device[i].y = FFT_out_device[i].y*w_cor_FFT_device[batch_i];
...
__device__
double cal_mu_cor_device(int *na_device, int *nb_device, int *nc_device, int *k_gas_device, double *sigma_device, double *epsilon_device, 
	double *diameter_HS_device, double *tot_rho_device, double *sigma_x3_device, double *epsilon_x_device, double *HS_diameter_3_device,
	double *temperature_device, double *MBWR_x_device, double *MBWR_gamma_device, double *MBWR_a_device, double *MBWR_b_device, 
	double *MBWR_c_device, double *MBWR_d_device, double *MBWR_F_device, double *MBWR_G_device, double *ncor_separate_device, 
	double *ncor_separate_device_temp_save, int i)
{
	int j, jj;
	double F_r, P_r, U_r;
	...
	{
		if (ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]]<0)
		{
			ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]] = 0;
		}
		tot_rho_device[i] += ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]];
	}




	if (tot_rho_device[i]<1e-12)
	{
		for (j=0; j<k_gas_device[0]; j++)
		{
			ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]] = 0;
		}
	}
	else
	{
		sigma_x3_device[i] = 0;
		epsilon_x_device[i] = 0;
		HS_diameter_3_device[i] = 0;
		for (j=0; j<k_gas_device[0]; j++)
		{
			for (jj=0; jj<k_gas_device[0]; jj++)
			{
				sigma_x3_device[i] += ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]]
									*ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]
									*pow((0.5*(sigma_device[j]+sigma_device[jj])),3);
				epsilon_x_device[i] +=  ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]]
									*ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]
									*sqrt(epsilon_device[j]*epsilon_device[jj])
									*pow((0.5*(sigma_device[j]+sigma_device[jj])),3);

				HS_diameter_3_device[i] += ncor_separate_device[i+j*na_device[0]*nb_device[0]*nc_device[0]]
									*ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]
									*pow((0.5*(diameter_HS_device[j]+diameter_HS_device[jj])),3);
			}
		}
		// printf("%lf\n", HS_diameter_3_device[i]);
		sigma_x3_device[i] = 1.0*sigma_x3_device[i]/tot_rho_device[i]/tot_rho_device[i];
		epsilon_x_device[i] = 1.0*epsilon_x_device[i]/tot_rho_device[i]/tot_rho_device[i]/sigma_x3_device[i];
		HS_diameter_3_device[i] = 1.0*HS_diameter_3_device[i]/tot_rho_device[i]/tot_rho_device[i];


		// if (i==1)
		// {
		// 	printf("%lf\t%lf\n", ncor_separate_device[i+0*na_device[0]*nb_device[0]*nc_device[0]], ncor_separate_device[i+1*na_device[0]*nb_device[0]*nc_device[0]]);
		// }
		...
		// {
		// 	printf("test\n");
		// 	printf("tot_rho_device: %lf\n", tot_rho_device[i]);
		// 	printf("sigma_x3_device: %lf\n", sigma_x3_device[i]);
		// 	printf("epsilon_x_device: %lf\n", epsilon_x_device[i]);
		// 	printf("HS_diameter_3_device: %lf\n", HS_diameter_3_device[i]);
		// }
		// // printf("%lf\t%lf\n", diameter_HS_device[0], diameter_HS_device[1]);
		// // 
		// // 
		// // 
		// // 
		// // rho_r = sigma_x3*tot_rho;
		// // (sigma_x3_device[i]*tot_rho_device[i])
  // //       T_r = 1.0*temperature[0]/epsilon_x;
		// // (1.0*temperature_device[0]/epsilon_x_device[i])

		// MBWR parameters of a
		MBWR_a_device[i*8+0] = MBWR_x_device[0]*(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[1]*pow((1.0*temperature_device[0]/epsilon_x_device[i]), 0.5) + MBWR_x_device[2] + MBWR_x_device[3]/(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[4]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2);
        MBWR_a_device[i*8+1] = MBWR_x_device[5]*(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[6] + MBWR_x_device[7]/(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[8]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2);
        MBWR_a_device[i*8+2] = MBWR_x_device[9]*(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[10] + MBWR_x_device[11]/(1.0*temperature_device[0]/epsilon_x_device[i]);
        MBWR_a_device[i*8+3] = MBWR_x_device[12];
        MBWR_a_device[i*8+4] = MBWR_x_device[13]/(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[14]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2);
        MBWR_a_device[i*8+5] = MBWR_x_device[15]/(1.0*temperature_device[0]/epsilon_x_device[i]);
        MBWR_a_device[i*8+6] = MBWR_x_device[16]/(1.0*temperature_device[0]/epsilon_x_device[i]) + MBWR_x_device[17]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2);
        MBWR_a_device[i*8+7] = MBWR_x_device[18]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2);

  //       if (i==1)
		// {
		// 	// printf("Tr:\t%lf\n", 1.0*temperature_device[0]/epsilon_x_device[i]);
		// 	printf("%lf\t%lf\n", MBWR_a_device[i*8+0], MBWR_a_device[i*8+4]);
		// }


        // MBWR parameters of b
        MBWR_b_device[i*6+0] = MBWR_x_device[19]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[20]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 3);
        MBWR_b_device[i*6+1] = MBWR_x_device[21]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[22]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 4);
        MBWR_b_device[i*6+2] = MBWR_x_device[23]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[24]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 3);
        MBWR_b_device[i*6+3] = MBWR_x_device[25]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[26]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 4);
        MBWR_b_device[i*6+4] = MBWR_x_device[27]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[28]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 3);
        MBWR_b_device[i*6+5] = MBWR_x_device[29]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 2) + MBWR_x_device[30]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 3) + MBWR_x_device[31]/pow((1.0*temperature_device[0]/epsilon_x_device[i]), 4);
  ...
  //       if (i==1)
		// {
		// 	// printf("Tr:\t%lf\n", 1.0*temperature_device[0]/epsilon_x_device[i]);
		// 	printf("%lf\t%lf\n", MBWR_b_device[i*6+0], MBWR_b_device[i*6+4]);
		// }


        // MBWR parameters of c
        MBWR_c_device[i*8+0] = MBWR_x_device[1]*sqrt((1.0*temperature_device[0]/epsilon_x_device[i]))*0.5 + MBWR_x_device[2] + 2*MBWR_x_device[3]/(1.0*temperature_device[0]/epsilon_x_device[i]) + 3*MBWR_x_device[4]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2);
        MBWR_c_device[i*8+1] = MBWR_x_device[6] + 2*MBWR_x_device[7]/(1.0*temperature_device[0]/epsilon_x_device[i]) + 3*MBWR_x_device[8]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2);
        MBWR_c_device[i*8+2] = MBWR_x_device[10] + 2*MBWR_x_device[11]/(1.0*temperature_device[0]/epsilon_x_device[i]);
        MBWR_c_device[i*8+3] = MBWR_x_device[12];
        MBWR_c_device[i*8+4] = 2*MBWR_x_device[13]/(1.0*temperature_device[0]/epsilon_x_device[i]) + 3*MBWR_x_device[14]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2);
        MBWR_c_device[i*8+5] = 2*MBWR_x_device[15]/(1.0*temperature_device[0]/epsilon_x_device[i]);
        MBWR_c_device[i*8+6] = 2*MBWR_x_device[16]/(1.0*temperature_device[0]/epsilon_x_device[i]) + 3*MBWR_x_device[17]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2);
        MBWR_c_device[i*8+7] = 3*MBWR_x_device[18]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2);


  //       if (i==1)
		// {
		// 	printf("MBWR_c_device: %lf\t%lf\n", MBWR_c_device[i*8+0], MBWR_c_device[i*8+4]);
		// }



		// MBWE parameters of d
        MBWR_d_device[i*6+0] = 3*MBWR_x_device[19]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 4*MBWR_x_device[20]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),3);
        MBWR_d_device[i*6+1] = 3*MBWR_x_device[21]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 5*MBWR_x_device[22]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),4);
        MBWR_d_device[i*6+2] = 3*MBWR_x_device[23]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 4*MBWR_x_device[24]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),3);
        MBWR_d_device[i*6+3] = 3*MBWR_x_device[25]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 5*MBWR_x_device[26]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),4);
        MBWR_d_device[i*6+4] = 3*MBWR_x_device[27]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 4*MBWR_x_device[28]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),3);
        MBWR_d_device[i*6+5] = 3*MBWR_x_device[29]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),2) + 4*MBWR_x_device[30]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),3) + 5*MBWR_x_device[31]/pow((1.0*temperature_device[0]/epsilon_x_device[i]),4);


  //       if (i==1)
		// {
		// 	printf("MBWR_d_device: %lf\t%lf\n", MBWR_d_device[i*6+0], MBWR_d_device[i*6+4]);
		// }



        // MBWR parameter of G
        MBWR_F_device[i] = exp(-MBWR_gamma_device[0]*pow(sigma_x3_device[i]*tot_rho_device[i],2));
        MBWR_G_device[i*6+0] = (1-MBWR_F_device[i])/(2*MBWR_gamma_device[0]);
        for (j=1; j<6; j++)
        {
            MBWR_G_device[i*6+j] = j/MBWR_gamma_device[0]*MBWR_G_device[i*6+j-1] - MBWR_F_device[i]/(2*MBWR_gamma_device[0])*pow((sigma_x3_device[i]*tot_rho_device[i]),2*j);
        }

  //       if (i==1)
		// {
		// 	printf("MBWR_G_device: %.5e\t%.5e\n", MBWR_G_device[i*6+0], MBWR_G_device[i*6+4]);
		// }
        ...
        for (j=0; j<=7; j++)
        {
            F_r += MBWR_a_device[i*8+j]*pow((sigma_x3_device[i]*tot_rho_device[i]), (j+1))/(j+1);
        }
        for (j=0; j<=5; j++)
        {
            F_r += MBWR_b_device[i*6+j]*MBWR_G_device[i*6+j];
        }
  ...
  //       if (i==1)
        ...
        for (j=0; j<=7; j++)
        {
            P_r += MBWR_a_device[i*8+j]*pow((sigma_x3_device[i]*tot_rho_device[i]), (j+2));
        }
        for (j=0; j<=5; j++)
        {
            P_r += MBWR_F_device[i]*MBWR_b_device[i*6+j]*pow((sigma_x3_device[i]*tot_rho_device[i]), (2*j+3));
        }
        P_r += (sigma_x3_device[i]*tot_rho_device[i])*(1.0*temperature_device[0]/epsilon_x_device[i]);
  ...
  //       if (i==1)
        ...
        for (j=0; j<=7; j++)
        {
            U_r += MBWR_c_device[i*8+j]*pow((sigma_x3_device[i]*tot_rho_device[i]), (j+1))/(j+1);
        }
        for (j=0; j<=5; j++)
        {
            U_r += MBWR_d_device[i*6+j]*MBWR_G_device[i*6+j];
        }
...
// // rho_r = sigma_x3*tot_rho;
  ...
  // //       T_r = 1.0*temperature[0]/epsilon_x;
		...
		//y = 1.0*tot_rho*PI*HS_diameter_3/6.0;
		// (1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)
		...
		for (j=0; j<k_gas_device[0]; j++)
		{
			...
			for (jj=0; jj<k_gas_device[0]; jj++)
			{
				deriv3 += ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]*pow((0.5*(sigma_device[j]+sigma_device[jj])),3);
				
				deriv1 += ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]*sqrt(epsilon_device[j]*epsilon_device[jj])
						*pow((0.5*(sigma_device[j]+sigma_device[jj])),3);
			}
			...
			// 	printf("deriv3: %.5e\n", deriv3);
			// }
			deriv3 = deriv3/tot_rho_device[i];
			deriv3 = deriv3 - sigma_x3_device[i];
			deriv3 = 2.0*deriv3/tot_rho_device[i];

			deriv1 = deriv1/tot_rho_device[i]/sigma_x3_device[i] - epsilon_x_device[i];
            deriv1 = 2.0*deriv1/tot_rho_device[i];
            deriv1 = -epsilon_x_device[i]*deriv3/sigma_x3_device[i] + deriv1;
   ...
   //          if (i==1)
			...
			deriv2 = (1.0*P_r/pow((sigma_x3_device[i]*tot_rho_device[i]),2) - (1.0*temperature_device[0]/epsilon_x_device[i])/(sigma_x3_device[i]*tot_rho_device[i])) 
					*(sigma_x3_device[i]+tot_rho_device[i]*deriv3);
        	deriv2 = deriv2 - (F_r-U_r)*deriv1/epsilon_x_device[i];
   ...
   //          if (i==1)
			// {
			// 	printf("LJ: %.5e\n", ((F_r*epsilon_x_device[i] + F_r*tot_rho_device[i]*deriv1 + tot_rho_device[i]*epsilon_x_device[i]*deriv2)/temperature_device[0]));
			// }
            ...
            for (jj=0; jj<k_gas_device[0]; jj++)
			{
				...
				ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]] += 
				(1.0*ncor_separate_device[i+jj*na_device[0]*nb_device[0]*nc_device[0]]/tot_rho_device[i])*pow((0.5*(diameter_HS_device[j]+diameter_HS_device[jj])),3);
			}
			// if (i==1)
			// {
			// 	printf("HS: %.5e\n", ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]]);
			// }
			ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]] = (1.0*(ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]]-HS_diameter_3_device[i])*PI/3.0 
				+ 1.0*PI*HS_diameter_3_device[i]/6.0)*tot_rho_device[i]*(1.0*(4-2*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0))/pow((1-(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)),3));
   //          if (i==1)
			// {
			// 	printf("HS: %.5e\n", ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]]);
			// }
			ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]] += 1.0*(4*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)-3*pow((1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0),2))
			/pow((1-(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)),2);
			// if (i==1)
			// {
			// 	printf("HS: %.5e\n", (ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]]));
			// }
			...
			// if (i==1)
			// {
			// 	printf("MF: %.5e\n", ((deriv1*pow(tot_rho_device[i],2)*sigma_x3_device[i]+2*epsilon_x_device[i]*tot_rho_device[i]*sigma_x3_device[i]+epsilon_x_device[i]*pow(tot_rho_device[i],2)*deriv3)
   //          /temperature_device[0]*PI*-16.0/9.0));
			// }
            ...
            // COR
            // COR
			ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]] = ((F_r*epsilon_x_device[i] + F_r*tot_rho_device[i]*deriv1 + tot_rho_device[i]*epsilon_x_device[i]*deriv2)/temperature_device[0])
			- (ncor_separate_device_temp_save[i+j*na_device[0]*nb_device[0]*nc_device[0]])
			- ((deriv1*pow(tot_rho_device[i],2)*sigma_x3_device[i]+2*epsilon_x_device[i]*tot_rho_device[i]*sigma_x3_device[i]+epsilon_x_device[i]*pow(tot_rho_device[i],2)*deriv3)
            /temperature_device[0]*PI*-16.0/9.0);
...
__global__
void cal_F_deri(int *na_device, int *nb_device, int *nc_device, int *k_gas_device,
	double *sigma_device, double *epsilon_device, 
	double *diameter_HS_device,  
	double *tot_rho_device,
	double *sigma_x3_device, double *epsilon_x_device, double *HS_diameter_3_device,
	double *temperature_device, double *MBWR_x_device, double *MBWR_gamma_device, double *MBWR_a_device, double *MBWR_b_device, 
	double *MBWR_c_device, double *MBWR_d_device,
	double *MBWR_F_device, double *MBWR_G_device, 
	...
	cufftDoubleComplex *nv1x_FFT_in_device, cufftDoubleComplex *nv1y_FFT_in_device, cufftDoubleComplex *nv1z_FFT_in_device, 
	cufftDoubleComplex *nv2x_FFT_in_device, cufftDoubleComplex *nv2y_FFT_in_device, cufftDoubleComplex *nv2z_FFT_in_device,
	double *ncor_separate_device, double *ncor_separate_device_temp_save)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i;
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]); i+=stride)
	{
		...
		if (n0_FFT_out_device[i].x<0)
		{
			n0_FFT_out_device[i].x = 0;
			// if (tn0<-0.0001)
			// {
		...
		cal_mu_cor_device(na_device, nb_device, nc_device, k_gas_device, sigma_device, epsilon_device, diameter_HS_device,  
			tot_rho_device, sigma_x3_device, epsilon_x_device, HS_diameter_3_device, temperature_device, MBWR_x_device, 
			MBWR_gamma_device, MBWR_a_device, MBWR_b_device, MBWR_c_device, MBWR_d_device, MBWR_F_device, MBWR_G_device,
			ncor_separate_device, ncor_separate_device_temp_save, i);
		...
		else
		{
			...
			n3_FFT_in_device[i].x = ((1.0*tn30*t3n3/(18.0*PI))+(1.0-3*tn3+1.0/tn32)*t2n3*tn33/(36.0*PI))*(3*tn2*tnv22-tn2*tn2*tn2)
	            					+ n0_FFT_out_device[i].x*tn31 + (tn1*tn2-tnv21)*tn32 + 80.0*exp((tn3-0.7)*80.0);
			n3_FFT_in_device[i].y = 0;
...
void sum_F_deri(int *na_device, int *nb_device, int *nc_device, double *diameter_HS_device, int *k_gas_device, 
	...
	cufftDoubleComplex *nv1x_FFT_out_device, cufftDoubleComplex *nv1y_FFT_out_device, cufftDoubleComplex *nv1z_FFT_out_device, 
	cufftDoubleComplex *nv2x_FFT_out_device, cufftDoubleComplex *nv2y_FFT_out_device, cufftDoubleComplex *nv2z_FFT_out_device, 
	double *ncor_separate_device, double *ncor_separate_imagine_device,
	double *w_n0_FFT_device, double *w_n3_FFT_device, double *w_nv2x_FFT_device, double *w_nv2y_FFT_device, double *w_nv2z_FFT_device, 
	double *w_cor_FFT_device, cufftDoubleComplex *ncor_FFT_in_device,
	int j)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]); i+=stride)
	{
		...
		ncor_FFT_in_device[i].x = ncor_separate_device[batch_i]*w_cor_FFT_device[batch_i];
		ncor_FFT_in_device[i].y = ncor_separate_imagine_device[batch_i]*w_cor_FFT_device[batch_i];
	}
}
...
__device__
double cal_F_cor_device(int *na_device, int *nb_device, int *nc_device, int *k_gas_device, double *sigma_device, double *epsilon_device, 
	double *diameter_HS_device,  double *tot_rho_device, double *sigma_x3_device, double *epsilon_x_device, double *HS_diameter_3_device,
	double *temperature_device, double *MBWR_x_device, double *MBWR_gamma_device, double *MBWR_a_device, double *MBWR_b_device, 
	double *MBWR_F_device, double *MBWR_G_device, double *ncor_separate_device, int i)
{
	// if (i>=na_device[0]*nb_device[0]*nc_device[0])
	// {
	// 	printf("FATAL ERROR!!!!!!!!!\n");
	// }
	double result = 0;
  ...
  //       if (i==1)
        ...
        // MBWR_G[0] = (1-MBWR_F[0])/(2*MBWR_gamma[0]);
        // for (i=1; i<6; i++)
        // {
        //     MBWR_G[i] = i/MBWR_gamma[0]*MBWR_G[i-1] - MBWR_F[0]/(2*MBWR_gamma[0])*pow(rho_r,2*i);
        // }
  //       if (i==1)
		// {
		// 	// printf("Tr:\t%lf\n", 1.0*temperature_device[0]/epsilon_x_device[i]);
  ...
  //       if (i==1)
		...
		for (j=0; j<=7; j++)
        {
            result +=  MBWR_a_device[i*8+j]*pow((sigma_x3_device[i]*tot_rho_device[i]), (j+1))/(j+1);
        }
        for (j=0; j<=5; j++)
        {
            result +=  MBWR_b_device[i*6+j]*MBWR_G_device[i*6+j];
        }
        // sum and convert to non-reduced unit
        // LJ TERM
        result = result*epsilon_x_device[i]/temperature_device[0];
  //       if (i==1)
		// {
		// 	printf("LJ: %lf\n", result);

		// 	// printf("HS: %lf\n", 1.0*(4*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)-3*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0))
	 //  //       /pow((1-(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)),2));
		// 	// printf("MF: %lf\n", (-16.0*PI*(epsilon_x_device[i]/temperature_device[0])*pow(tot_rho_device[i],2)*sigma_x3_device[i]/9.0));
		// }


		result = result*tot_rho_device[i] 
				- (1.0*(4*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)-3*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)*(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0))
	        /pow((1-(1.0*tot_rho_device[i]*PI*HS_diameter_3_device[i]/6.0)),2))*tot_rho_device[i]
				- (-16.0*PI*(epsilon_x_device[i]/temperature_device[0])*pow(tot_rho_device[i],2)*sigma_x3_device[i]/9.0);
  ...
  //       F_HS = ;
  //       // MF
  //       F_MF = -16.0*PI*(epsilon_x/temperature[0])*pow(tot_rho,2)*sigma_x3/9.0;
  //       // COR
  //       F_cor = F_LJ*tot_rho - F_HS*tot_rho - F_MF;
...
void cal_grand_potential(int *na_device, int *nb_device, int *nc_device, int *k_gas_device, double *Vext_device, 
	double *sigma_device, double *epsilon_device, double *diameter_HS_device, double *tot_rho_device, double *sigma_x3_device,
	double *epsilon_x_device, double *HS_diameter_3_device, double *temperature_device, double *MBWR_x_device, double *MBWR_gamma_device, 
	double *MBWR_a_device, double *MBWR_b_device, double *MBWR_F_device, double *MBWR_G_device, 
	double *chem_device,  double *sqrt_dens_device, double *rho_bulk_device, cufftDoubleComplex *n0_FFT_out_device, 
	cufftDoubleComplex *n1_FFT_out_device, cufftDoubleComplex *n2_FFT_out_device, cufftDoubleComplex *n3_FFT_out_device, 
	cufftDoubleComplex *nv1x_FFT_out_device, cufftDoubleComplex *nv1y_FFT_out_device, cufftDoubleComplex *nv1z_FFT_out_device, 
	cufftDoubleComplex *nv2x_FFT_out_device, cufftDoubleComplex *nv2y_FFT_out_device, cufftDoubleComplex *nv2z_FFT_out_device,
	double *ncor_separate_device, double *denskr_device, double *grand_pontential_device)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i, j;
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]); i+=stride)
	{
		...
		// 	printf("test\n");
		// 	double temp;
		// 	temp = cal_F_cor_device(na_device, nb_device, nc_device, k_gas_device, sigma_device, epsilon_device, diameter_HS_device, 
		// 		tot_rho_device, sigma_x3_device, epsilon_x_device, HS_diameter_3_device, temperature_device,
		// 		ncor_separate_device, i);
		// 	printf("%lf\n", temp);
		// 	// cal_F_cor_device(na_device, ncor_separate_device);
		// }
		...
		if (tn3>=1e-5)
		{
            ...
            grand_pontential_device[i] += cal_F_cor_device(na_device, nb_device, nc_device, k_gas_device, sigma_device, epsilon_device, diameter_HS_device, 
				tot_rho_device, sigma_x3_device, epsilon_x_device, HS_diameter_3_device, temperature_device, MBWR_x_device, MBWR_gamma_device,
				MBWR_a_device, MBWR_b_device, MBWR_F_device, MBWR_G_device, ncor_separate_device, i);
		}
	}
}
...
void cal_grad(int *na_device, int *nb_device, int *nc_device, double *chem_device, cufftDoubleComplex *FFT_out_device, 
	double *denskr_device, double *Vext_device, double *sqrt_dens_device, double *rho_bulk_device, cufftDoubleComplex *ncor_FFT_out_device,
	double *gradient_device, int j)
{
	int index = blockIdx.x * blockDim.x + threadIdx.x;
	int stride = blockDim.x * gridDim.x;
	int i;
	...
	for (i=index; i<(na_device[0]*nb_device[0]*nc_device[0]); i+=stride)
	{
		...
		{
			gradient_device[batch_i] = log(sqrt_dens_device[batch_i]*sqrt_dens_device[batch_i]) + Vext_device[batch_i] - chem_device[j] + FFT_out_device[i].x + denskr_device[batch_i] + ncor_FFT_out_device[i].x;
...
double myvalue(double *sqrt_dens, long int system_size)
{
	...
	// cudaMallocHost(&temp, sizeof(cufftDoubleComplex)*na[0]*nb[0]*nc[0]);
	// int temp_i;
    ...
    for (j=0; j<k_gas[0]; j++)
    {
    	...
    	// cudaMemcpy(temp, FFT_in_device, sizeof(cufftDoubleComplex)*na[0]*nb[0]*nc[0], cudaMemcpyDeviceToHost);
    	// for (temp_i=0; temp_i<na[0]*nb[0]*nc[0]; temp_i++)
    	// {
    	// 	printf("%lf\t%lf\n", temp[i].x, temp[i].y);
    	// }
		...
		nv2x_FFT_in_device, nv2y_FFT_in_device, nv2z_FFT_in_device, ncor_FFT_in_device, w_n0_FFT_device, w_n3_FFT_device, 
		w_nv2x_FFT_device, w_nv2y_FFT_device, w_nv2z_FFT_device, w_cor_FFT_device, uatt_device, denskr_device, denski_device, j);

		// check_complex<<<1,32>>>(na[0]*nb[0]*nc[0], ncor_FFT_in_device);

		cufftExecZ2Z(fft_plan_device, ncor_FFT_in_device, ncor_FFT_out_device, CUFFT_INVERSE);

		complext_split_into_real_only_separate_batch<<<(int)((na[0]*nb[0]*nc[0]-1)/running_block_size+1),running_block_size>>>
		(na[0]*nb[0]*nc[0], ncor_separate_device, ncor_FFT_out_device, j);
    }

    // check_double<<<1,32>>>(na[0]*nb[0]*nc[0]*k_gas[0], ncor_separate_device);
    ...
    // calculate grand potential for hard sphere part
    cal_grand_potential<<<(int)((na[0]*nb[0]*nc[0]-1)/running_block_size+1),running_block_size>>>
    (na_device, nb_device, nc_device, k_gas_device, Vext_device, sigma_device,epsilon_device, diameter_HS_device, tot_rho_device,
    sigma_x3_device, epsilon_x_device, HS_diameter_3_device, temperature_device, MBWR_x_device, MBWR_gamma_device, 
   	MBWR_a_device, MBWR_b_device, MBWR_F_device, MBWR_G_device, chem_device, sqrt_dens_device, rho_bulk_device, n0_FFT_out_device, 
	n1_FFT_out_device, n2_FFT_out_device, n3_FFT_out_device, nv1x_FFT_out_device, nv1y_FFT_out_device, nv1z_FFT_out_device, 
	nv2x_FFT_out_device, nv2y_FFT_out_device, nv2z_FFT_out_device, ncor_separate_device, denskr_device, grand_pontential_device);
	...
	// reduction_sum_size = na[0]*nb[0]*nc[0];
	// reduction_cal_array = grand_pontential_device;
	// while (1)
	// {
	// 	reduction_sum_numBlocks = (reduction_sum_size-1)/running_block_size + 1;
	// 	fake_N = ((reduction_sum_size-1)/running_block_size + 1)*running_block_size;
	// 	// cudaMalloc((void **)&reduction_sum_array, reduction_sum_numBlocks*sizeof(double));
	// 	// cudaMalloc((void **)&tmp_reduction_sum, sizeof(int)*fake_N);
	// 	sum<<<reduction_sum_numBlocks, running_block_size>>>
	// 	(reduction_sum_size, fake_N, reduction_cal_array, reduction_sum_array, tmp_reduction_sum);
	// 	// cudaFree(reduction_cal_array);
	// 	if (reduction_sum_numBlocks==1)
	// 	{
	// 		break;
	// 	}
	// 	reduction_sum_size = reduction_sum_numBlocks;
	// 	reduction_cal_array = reduction_sum_array;
	// }
	// cudaMemcpy(transfer_result, reduction_sum_array, sizeof(double), cudaMemcpyDeviceToHost);
	// // copy<<<reduction_sum_numBlocks, running_block_size>>>
	// // 	(reduction_sum_size, fake_N, reduction_cal_array, reduction_sum_array, tmp_reduction_sum);
	// cudaFree(reduction_sum_array);
	// result = transfer_result[0]* (la_expanded[0]/(na[0])) * (lb_expanded[0]/(nb[0])) * (lc_expanded[0]/(nc[0]));
	// printf("test free energy: %lf\n", result);
...
void mygrad(double *gradient, double *sqrt_dens, long int system_size)
{
    ...
    (na_device, nb_device, nc_device, k_gas_device, sigma_device, epsilon_device, diameter_HS_device, tot_rho_device, 
  	sigma_x3_device, epsilon_x_device, HS_diameter_3_device, temperature_device, MBWR_x_device, MBWR_gamma_device, 
  	MBWR_a_device, MBWR_b_device, MBWR_c_device, MBWR_d_device, MBWR_F_device, MBWR_G_device, 
  	n0_FFT_out_device, n1_FFT_out_device, n2_FFT_out_device, n3_FFT_out_device, 
	...
	nv2x_FFT_in_device, nv2y_FFT_in_device, nv2z_FFT_in_device, ncor_separate_device, ncor_separate_device_temp_save);

	// check_double<<<1,32>>>(na[0]*nb[0]*nc[0]*k_gas[0], ncor_separate_device_temp_save);
	...
	for (j=0; j<k_gas[0]; j++)
    {
    	real_split_into_complex_cor_batch<<<(int)((na[0]*nb[0]*nc[0]-1)/running_block_size+1),running_block_size>>>
    	(na[0]*nb[0]*nc[0], ncor_separate_device_temp_save, FFT_in_device, j);

    	// check_complex<<<1,32>>>(na[0]*nb[0]*nc[0], FFT_in_device);

    	if (cufftExecZ2Z(fft_plan_device, FFT_in_device, FFT_out_device, CUFFT_FORWARD) != CUFFT_SUCCESS)
		{
			printf("CUFFT error: Backward FFT of density FAILED at specie:\n");
		}
		// check_complex<<<1,32>>>(na[0]*nb[0]*nc[0], FFT_out_device);

		complext_split_into_real_separate_batch<<<(int)((na[0]*nb[0]*nc[0]-1)/running_block_size+1),running_block_size>>>
		(na[0]*nb[0]*nc[0], ncor_separate_device, ncor_separate_imagine_device, FFT_out_device, j);
    }

    // check_double<<<1,32>>>(na[0]*nb[0]*nc[0]*k_gas[0], ncor_separate_imagine_device);
	...
	for (j=0; j<k_gas[0]; j++)
    {
		...
		n1_FFT_out_device, n2_FFT_out_device, n3_FFT_out_device, nv1x_FFT_out_device, nv1y_FFT_out_device, nv1z_FFT_out_device, 
		nv2x_FFT_out_device, nv2y_FFT_out_device, nv2z_FFT_out_device, 
		ncor_separate_device, ncor_separate_imagine_device,
		w_n0_FFT_device, w_n3_FFT_device, w_nv2x_FFT_device, 
		w_nv2y_FFT_device, w_nv2z_FFT_device, 
		w_cor_FFT_device, ncor_FFT_in_device,
		j);
		...
		if (cufftExecZ2Z(fft_plan_device, ncor_FFT_in_device, ncor_FFT_out_device, CUFFT_INVERSE) != CUFFT_SUCCESS)
		{
			printf("CUFFT error: Forward FFT of density FAILED at specie:\t%d\n", i);
		}

		// check_complex<<<1,32>>>(na[0]*nb[0]*nc[0], ncor_FFT_out_device);


		cal_grad<<<(int)((na[0]*nb[0]*nc[0]-1)/running_block_size+1),running_block_size>>>
		(na_device, nb_device, nc_device, chem_device, FFT_out_device, denskr_device, Vext_device, sqrt_dens_device, rho_bulk_device, ncor_FFT_out_device, 
			gradient_device, j);
    }

    // check_double<<<1,32>>>(na[0]*nb[0]*nc[0]*k_gas[0], gradient_device);
...
int main(int argc, char *argv[])
{
    ...
    cudaMalloc((void **)&temperature_device, sizeof(double)*1);
    ...
    cudaMalloc((void **)&epsilon_device, sizeof(double)*k_gas[0]);
    ...
    cudaMemcpy(epsilon_device, epsilon, sizeof(double)*k_gas[0], cudaMemcpyHostToDevice);
    ...
    cudaMalloc((void **)&w_cor_FFT_device, sizeof(double)*na[0]*nb[0]*nc[0]*k_gas[0]);
    ...
    diameter_HS_device, w_n0_FFT_device, w_n3_FFT_device, w_nv2x_FFT_device, w_nv2y_FFT_device, w_nv2z_FFT_device, w_cor_FFT_device);
    ...
    MBWR_x = (double *) malloc(sizeof(double)*32);
    MBWR_gamma = (double *) malloc(sizeof(double));

    cudaMalloc((void **)&MBWR_x_device, sizeof(double)*32);
    cudaMalloc((void **)&MBWR_gamma_device, sizeof(double)*32);
    // MBWR_a = (double *) malloc(sizeof(double)*8);
    // MBWR_b = (double *) malloc(sizeof(double)*6);
    // MBWR_c = (double *) malloc(sizeof(double)*8);
    // MBWR_d = (double *) malloc(sizeof(double)*6);
    // MBWR_G = (double *) malloc(sizeof(double)*6);
    // MBWR_gamma = (double *) malloc(sizeof(double));
    // MBWR_F = (double *) malloc(sizeof(double));
    // MBWR parameter from Keith E. Gubbins
    MBWR_x[0] = 0.8623085097507421;
    MBWR_x[1] = 2.976218765822098;
    MBWR_x[2] = -8.402230115796038;
    MBWR_x[3] = 0.1054136629203555;
    MBWR_x[4] = -0.8564583828174598;
    MBWR_x[5] = 1.582759470107601;
    MBWR_x[6] = 0.7639421948305453;
    MBWR_x[7] = 1.753173414312048;
    MBWR_x[8] = 2.798291772190376e+03;
    MBWR_x[9] = -4.8394220260857657e-02;
    MBWR_x[10] = 0.9963265197721935;
    MBWR_x[11] = -3.698000291272493e+01;
    MBWR_x[12] = 2.084012299434647e+01;
    MBWR_x[13] = 8.305402124717285e+01;
    MBWR_x[14] = -9.574799715203068e+02;
    MBWR_x[15] = -1.477746229234994e+02;
    MBWR_x[16] = 6.398607852471505e+01;
    MBWR_x[17] = 1.603993673294834e+01;
    MBWR_x[18] = 6.805916615864377e+01;
    MBWR_x[19] = -2.791293578795945e+03;
    MBWR_x[20] = -6.245128304568454;
    MBWR_x[21] = -8.116836104958410e+03;
    MBWR_x[22] = 1.488735559561229e+01;
    MBWR_x[23] = -1.059346754655084e+04;
    MBWR_x[24] = -1.131607632802822e+02;
    MBWR_x[25] = -8.867771540418822e+03;
    MBWR_x[26] = -3.986982844450543e+01;
    MBWR_x[27] = -4.689270299917261e+03;
    MBWR_x[28] = 2.593535277438717e+02;
    MBWR_x[29] = -2.694523589434903e+03;
    MBWR_x[30] = -7.218487631550215e+02;
    MBWR_x[31] = 1.721802063863269e+02;
    MBWR_gamma[0] = 3;

    cudaMemcpy(MBWR_x_device, MBWR_x, sizeof(double)*32, cudaMemcpyHostToDevice);
    cudaMemcpy(MBWR_gamma_device, MBWR_gamma, sizeof(double), cudaMemcpyHostToDevice);

    cudaMalloc((void **)&tot_rho_device, sizeof(double)*na[0]*nb[0]*nc[0]);
    cudaMalloc((void **)&sigma_x3_device, sizeof(double)*na[0]*nb[0]*nc[0]);
    cudaMalloc((void **)&epsilon_x_device, sizeof(double)*na[0]*nb[0]*nc[0]);
    cudaMalloc((void **)&HS_diameter_3_device, sizeof(double)*na[0]*nb[0]*nc[0]);

    cudaMalloc((void **)&MBWR_a_device, sizeof(double)*na[0]*nb[0]*nc[0]*8);
    cudaMalloc((void **)&MBWR_b_device, sizeof(double)*na[0]*nb[0]*nc[0]*6);
    cudaMalloc((void **)&MBWR_c_device, sizeof(double)*na[0]*nb[0]*nc[0]*8);
    cudaMalloc((void **)&MBWR_d_device, sizeof(double)*na[0]*nb[0]*nc[0]*6);
    cudaMalloc((void **)&MBWR_F_device, sizeof(double)*na[0]*nb[0]*nc[0]*1);
    cudaMalloc((void **)&MBWR_G_device, sizeof(double)*na[0]*nb[0]*nc[0]*6);
    ...
    cudaMalloc((void **)&ncor_FFT_in_device, sizeof(cufftDoubleComplex)*na[0]*nb[0]*nc[0]);
    ...
    cudaMalloc((void **)&ncor_FFT_out_device, sizeof(cufftDoubleComplex)*na[0]*nb[0]*nc[0]);
    ...
    cudaMalloc((void **)&ncor_separate_device, sizeof(double)*na[0]*nb[0]*nc[0]*k_gas[0]);
    cudaMalloc((void **)&ncor_separate_imagine_device, sizeof(double)*na[0]*nb[0]*nc[0]*k_gas[0]);
    cudaMalloc((void **)&ncor_separate_device_temp_save, sizeof(double)*na[0]*nb[0]*nc[0]*k_gas[0]);
    ...
    int output_signal;
    output_signal = cg_descent(sqrt_dens, system_size, NULL, NULL, 1.e-6, myvalue, mygrad, myvalgrad, NULL) ;
    cudaDeviceSynchronize();
    // printf("%s\t", argv[1]);
