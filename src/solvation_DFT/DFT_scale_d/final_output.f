      ...
      DO i = 1, nfft1
        DO j = 1, nfft2
          DO k = 1, nfft3
		   ...
		   if(rho_O_old(i,j,k).gt.dft_peak)then
		      dft_peak=rho_O_old(i,j,k)
		   endif
	...
	close(400)
	  ...
	  if(scale_md.and.(.not.one_step))then
	    ...
	    if(dft_peak.lt.1.0)then
	      ...
	      sc_rate=log((md_peak-1.0)/(dft_peak-1.0))/dft_peak
		  DO i = 1, nfft1
            DO j = 1, nfft2
              DO k = 1, nfft3
			    x=rho_O_old(i,j,k)
				rho_O_old(i,j,k)=1.0+(x-1.0)*exp(sc_rate*x)
				x=rho_H_old(i,j,k)
				rho_H_old(i,j,k)=1.0+(x-1.0)*exp(sc_rate*x)
			  enddo
			enddo
		  enddo
		endif
